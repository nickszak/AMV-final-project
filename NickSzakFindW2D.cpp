...
//	while ( inFile >> w[i] >> rs[0][i] >> rs[1][i] >> rs[2][i] >> rs[3][i] >> rs[4][i++] )
	...
	for (i = 0; i < n_wav; i++) {  // use max index = 880 for wavelengths below 700 nm else use n_wav
		...
		for (z = 0; z < n_wav; z++)
			...
			{ 
				...
				for (j = 0; j < 4; j++)
					for (k = j + 1; k < 5; k++) {
						tempD = sqrt((rs[j][i] - rs[k][i])*(rs[j][i] - rs[k][i]) + (rs[j][z] - rs[k][z])*(rs[j][z] - rs[k][z]));
				...
				if (minD > maxMinD) {
					maxMinD = minD;
					i_maxMinD = i;
					z_maxMinD = z; // save corresponding wavelength index
	...
	if (i_maxMinD != 1201) {
		cout << "Optimal wavelength is " << w[i_maxMinD]<< ",  " << w[z_maxMinD] << endl;
		maxD = 0.0;
		minD = 4096.0;
		for (j = 0; j < 4; j++) // to verify obtained results
			for (k = j + 1; k < 5; k++) {
				tempD = sqrt((rs[j][i_maxMinD] - rs[k][i_maxMinD])*(rs[j][i_maxMinD] - rs[k][i_maxMinD]) + (rs[j][z_maxMinD] - rs[k][z_maxMinD])*(rs[j][z_maxMinD] - rs[k][z_maxMinD]));
				if (tempD < minD) minD = tempD;
				if (tempD > maxD) maxD = tempD;
			}  // end of nested j,k loops
