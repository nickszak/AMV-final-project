...
//	file_dir = ".\\images\\";
	...
	int distX, distY, distZ, distXYZ, distAvgX, distAvgY, distAvgZ, distAvgXYZ;
	...
	distX = ((maxG)* cos(((maxB * 360) / 255))); //convert X from cylindrical 
	distY = ((maxG)* sin(((maxB * 360) / 255))); //convert Y from cylindrical
	distZ = maxR; //Z is scalar 
	distXYZ = sqrt((pow(distX, 2)) + (pow(distY, 2)) + (pow(distZ, 2))); //Distance

	distAvgX = ((avgG)* cos(((avgB * 360) / 255))); //average X
	distAvgY = ((avgG)* sin(((avgB * 360) / 255))); //average Y
	distAvgZ = avgR; //average Z
	distAvgXYZ = sqrt((pow(distAvgX, 2)) + (pow(distAvgY, 2)) + (pow(distAvgZ, 2))); //average Distance
	...
	cout << "Object's S statistics are (min, average, max, std): ( " << minR << " , " << avgR << ", " << maxR << ", " << stdR << " )" << endl;
	cout << "Object's L statistics are (min, average, max, std): ( " << minG << " , " << avgG << ", " << maxG << ", " << stdG << " )" << endl;
	cout << "Object's H statistics are (min, average, max, std): ( " << minB << " , " << avgB << ", " << maxB << ", " << stdB << " )" << endl;
	cout << "Object's DISTANCE statistics are (max, avg): ( " << distXYZ << ", " << distAvgXYZ << " )" << endl;
	...
	outFile << "Object's S statistics are (min, average, max, std): ( " << minR << " , " << avgR << ", " << maxR << ", " << stdR << " )" << endl;
	outFile << "Object's L statistics are (min, average, max, std): ( " << minG << " , " << avgG << ", " << maxG << ", " << stdG << " )" << endl;
	outFile << "Object's H statistics are (min, average, max, std): ( " << minB << " , " << avgB << ", " << maxB << ", " << stdB << " )" << endl;
	outFile << "Object's DISTANCE statistics are (max, avg): ( " << distXYZ << ", " << distAvgXYZ << " )" << endl;
...
{
	Mat src, dst, work, mask, HLS; // images defs
	...
	cvtColor(src, HLS, CV_BGR2HLS); //create Luv image
