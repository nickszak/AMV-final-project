...
//	while ( inFile >> w[i] >> rs[0][i] >> rs[1][i] >> rs[2][i] >> rs[3][i] >> rs[4][i++] )
	...
	for (i = 0; i < n_wav; i++) {  // use max index = 880 for wavelengths below 700 nm else use n_wav
     ...
     for (m =0; m< n_wav; m++)
			...
			{ 
				...
				for (j = 0; j < 4; j++)
					for (k = j + 1; k < 5; k++) {
						tempD = sqrt((rs[j][i] - rs[k][i])*(rs[j][i] - rs[k][i]) + (rs[j][z] - rs[k][z])*(rs[j][z] - rs[k][z])+ (rs[j][m] - rs[k][m])*(rs[j][m] - rs[k][m]));
				...
				if (minD > maxMinD) {
					...
					i_maxMinD = i;
					z_maxMinD = z;
          m_maxMinD = m; // save corresponding wavelength index
	...
	if (i_maxMinD != 1201) {
		cout << "Optimal wavelength is " << w[i_maxMinD]<< ",  " << w[z_maxMinD] << ",  " << w[m_maxMinD] << endl;
		maxD = 0.0;
		minD = 4096.0;
		for (j = 0; j < 4; j++) // to verify obtained results
			for (k = j + 1; k < 5; k++) {
				tempD = sqrt((rs[j][i_maxMinD] - rs[k][i_maxMinD])*(rs[j][i_maxMinD] - rs[k][i_maxMinD]) + (rs[j][z_maxMinD] - rs[k][z_maxMinD])*(rs[j][z_maxMinD] - rs[k][z_maxMinD])+ (rs[j][m_maxMinD] - rs[k][m_maxMinD])*(rs[j][m_maxMinD] - rs[k][m_maxMinD]));
				if (tempD < minD) minD = tempD;
				if (tempD > maxD) maxD = tempD;
			}  // end of nested j,k loops
